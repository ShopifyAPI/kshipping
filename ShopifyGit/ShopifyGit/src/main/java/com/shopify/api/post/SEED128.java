...
// reorder for little endian	  
	...
	//padding
	private byte[][] padding(byte[] dataByte) {
		int quotient = dataByte.length / 16;
		int remain = dataByte.length % 16;
		int loop = 0;
		
		byte[][] result;
		
		if(quotient==0) {
			result = new byte[1][16];
			loop = 1;
		}
		else if(remain==0) {
			result = new byte[quotient][16];
			loop = quotient;
		}
		else {
		...
		for(int i=0; i<loop; i++) {
			for(int j=0; j<16; j++) {
				point = (i * 16) + j;
				if(point <= dataByte.length-1) result[i][j] = dataByte[point];
				else result[i][j] = 0x00;
			}
		}
	...
	//2차원 byte 를 1차원 byte 로 변환
	private byte[] convertByteDimension(byte[][] dataByte) {
		byte[] result = new byte[dataByte.length * 16];
		
		int point;
		for(int i=0; i<dataByte.length; i++) {
			for(int j=0; j<16; j++) {
				point = (i * 16) + j;
				result[point] = dataByte[i][j];
			}
		}
	...
	//암호화
	public String getEncryptData(String key, String data) {
		byte[] pbUserKey = key.getBytes();
		byte[][] dataByte = padding(data.getBytes());
		
		//roundKey 생성
		int pdwRoundKey[] = SeedRoundKey(new int[pbUserKey.length*2], pbUserKey);
		
		byte[][] resultByte = new byte[dataByte.length][16];
		for(int i=0; i<dataByte.length; i++) {
			byte[] resultBuf = SeedEncrypt(dataByte[i], pdwRoundKey, new byte[16]);
			for(int j=0; j<16; j++) {
				resultByte[i][j] = resultBuf[j];
			}
		}
		
		byte[] result = convertByteDimension(resultByte);
		
		return byteArrayToHex(result);
	}
	
	//복호화
	public String getDecryptData(String key, String data) {
		byte[] dataBuf = hexToByteArray(data);
		
		byte[] pbUserKey = key.getBytes();
		byte[][] dataByte = padding(dataBuf);
		...
		byte[][] resultByte = new byte[dataByte.length][16];
		for(int i=0; i<dataByte.length; i++) {
			byte[] resultBuf = SeedDecrypt(dataByte[i], pdwRoundKey, new byte[16]);
			for(int j=0; j<16; j++) {
				resultByte[i][j] = resultBuf[j];
			}
		}
