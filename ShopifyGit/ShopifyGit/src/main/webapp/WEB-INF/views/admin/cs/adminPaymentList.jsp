...
<script type="text/javascript">
	...
	var bindEvent = function () {
        ...
        $(document).on("click",".list-edit",function(){
            ...
            var url = "/admin/cs/popup/adminPaymentShowPop?masterCode=" + masterCode + "&combineCode=" + combineCode;
            openPop(url, "", 1200, "", "", "");
            
           /*  var masterCode = $(this).closest("tr").data("idx");
            var url = "/admin/cs/popup/adminPaymentShowPop?masterCode=" + masterCode;
            openPop(url, "", 1200, "", "", ""); */
        });
	...
	var mailSend = function() {
        ...
        var confirm_1 = confirm(getMessage("alert.proc", ["common.target","button.send"]));//선택 항목을 수정 하시겠습니까?
        
		if (confirm_1 == true) {
			
			var type = "post";
			var url = "/admin/cs/adminSendMail";

			// 입력된 모든Element(을)를 문자열의 데이터에 serialize 한다.
			ajaxCall(type, url, param, sendFromMail);
	...
	var sendFromMail = function(data){
		
		if(data.errCode == true) {
		    alert(getMessage("alert.proc.end", ["button.send"]));  // 수정이 완료 되었습니다.
            location.reload();
		} else {
			alert(getMessage("alert.proc.err", ["button.send"]));  // 수정 중 오류가 발생했습니다.
		}
    ...
    var downExcel = function() {
    	location.replace("/admin/cs/adminPaymentListExcel");
    }
	...
	var updateStatus = function(status){
		...
		if (confirm_1 == true) {
			...
			var type = "post";
			var url = "/admin/cs/adminUpdateStatus";
           ...
           <div class="cont_body">
               <article>
                    ...
                    <ul class="tab">
                        ...
                        <li class="on"><a href="/admin/cs/adminPaymentList"><spring:message code="cs.payment" text="추가요금" /></a></li>
                    </ul>
                    
					<ul class="tab_conts">
                        <li>
                            <div class="tit_wrap mt0">
                            	<form name="searchform" method="get" id="searchform" action="/admin/cs/adminPaymentList">
                            		<div class="sect_right">
	                                    ...
	                                    <div class="searchbox"> <!--인풋박스에 클릭시 active 클래스 추가-->
	                                        <input type="text" name="searchWord" placeholder="<spring:message code="common.searchWord" text="검색어" />" value="${search.searchWord}">
                            ...
                            <div class="module_group">
                                <div class="btn_group">
                                    <button type="button" id="btn_paymentComplete" class="btn_type2"><spring:message code="shipment.list.payment.state.y" text="결제완료" /></button>
