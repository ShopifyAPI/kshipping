...
//            console.log(parameter)
        ...
        var setPageView = function(data){
        	var chkShop = data.statusShop;
        	var chkSeller = data.statusSeller;
        	var redirectUrl = data.redirectUrl;
        	//if(chk == "success")
        		console.log(chkShop);
        		console.log(chkSeller);
        	if(chkShop == "OK" && chkSeller == "OK"){
        		if(redirectUrl != "")
        			location.replace(redirectUrl);
        		else
        			  location.replace("/");
        	}else{
        		if(chkSeller == "BAD_REQUEST"){
        			alert(getMessage('error.login'));
        		}
        	}
        }
        //console.log($("#email").val());
        var key = getCookie("ShopifySaveIdKey");
    	$("#email").val(key); 
    	
    	if($("#email").val() != "undefined"){ // 그 전에 ID를 저장해서 처음 페이지 로딩 시, 입력 칸에 저장된 ID가 표시된 상태라면,
    		$("#saveId").attr("checked", true); // ID 저장하기를 체크 상태로 두기.
    	}
    	 
    	$("#saveId").change(function(){ // 체크박스에 변화가 있다면,
    		if($("#saveId").is(":checked")){ // ID 저장하기 체크했을 때,
    			setCookie("ShopifySaveIdKey", $("#email").val(), 7); // 7일 동안 쿠키 보관
    		}else{ // ID 저장하기 체크 해제 시,
    			deleteCookie("ShopifySaveIdKey");
    		}
    	});
        ...
        <div class="login">
            <div class="form_login">
                ...
                <div class="idbox" id="ajax-form">
                    <input type="text" id="email" name="email" maxlength="100" data-label="Email" data-required="Y" placeholder="Email"/>
                    <input type="password" id="password" name="password" maxlength="50" data-label="Password" data-required="Y" placeholder="Password" />
