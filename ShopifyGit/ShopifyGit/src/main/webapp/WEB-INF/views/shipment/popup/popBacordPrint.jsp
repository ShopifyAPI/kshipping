		...
		<div class="page">
			<div class="subpage" id="content">
				 ...
				 <table border=1>
                    ...
                    <c:if test="${ status.count % 2 == 0 }"><td width=400 align="right"></c:if>
                    <img src="${pageContext.request.contextPath }/shipment/popup/popCreateBarcode/${item.masterCode },${item.invoice }" width="285" height="83" >
                    <div style="font-size:0.5em">${item.invoice } ${item.orderName}</div>
                    <c:if test="${ status.count % 16 == 0 }"><div style="page-break-before:always"></div></c:if>
                    <c:if test="${ status.count % 2 == 1 }"></td></c:if>
                    <c:if test="${ status.count % 2 == 0 }"></td></tr><tr><td height="30px" colspan="2">&nbsp;</td></tr></c:if>
                    </c:forEach>
                </table>
                -->
				<!-- 영역 magin 확인용 칼라 테스트: 조한두 (20.06.24.수)-->
				<!--  
				<c:choose>
				    <c:when test="${ invoicesize == 1 }">
                    ...
                    <tr><td height="30px" colspan="2" style="background-color:red;" >&nbsp;</td></tr>
                    <c:forEach items="${invoiceList}" var="item" varStatus="status">
                    <c:if test="${ status.count % 2 == 1 }"><tr height=83 style="background-color:blue;"><td width=50%  align="center" style="background-color:yellow;"></c:if>
                    <c:if test="${ status.count % 2 == 0 }"><td width=50% align="center" style="background-color:green;"></c:if>
                    <img src="${pageContext.request.contextPath }/shipment/popup/popCreateBarcode/${item.masterCode },${item.invoice }" width="285" height="83" >
                    <div style="font-size:0.5em">${item.invoice}  ${item.orderName}</div>
                    <c:if test="${ status.count % 16 == 0 }"><div style="page-break-before:always"></div></c:if>
                    <c:if test="${ status.count % 2 == 1 }"></td></c:if>
                    <c:if test="${ status.count % 2 == 0 }"></td></tr><tr><td height="30px" colspan="2" style="background-color:blue;">&nbsp;</td></tr></c:if>
                    </c:forEach>
                </table>
               -->
				<!-- 운영용: Color 없음  -->
				<c:choose>
                    ...
                    <c:if test="${ status.count % 2 == 1 }"><tr height=83 ><td width=50%  align="center" ></c:if>
                    <c:if test="${ status.count % 2 == 0 }"><td width=50% align="center" ></c:if>
                    <img src="${pageContext.request.contextPath }/shipment/popup/popCreateBarcode/${item.masterCode },${item.invoice }" width="285" height="83" >
                    <div style="font-size:1.1em">${item.invoice} - ${item.buyerCountryCode}   ${item.orderName}</div>
                    
                    <c:if test="${ status.count % 2 == 1 }"></td></c:if>
                    <c:if test="${ status.count % 2 == 0 }"></td></tr><tr><td height="6px" colspan="2" >&nbsp;</td></tr></c:if>
                    <!-- 페이지 넘김 16개 이상 -->
